...
var EIP = (function(EIP, $, undefined) {
	EIP.pages.aliases = function() {
		EIP.sendRequest("getAliases+", {
			"account": EIP.account,
			"timestamp": 0
		}, function(response) {
			if (response.aliases && response.aliases.length) {
				...
				for (var i = 0; i < alias_count; i++) {
					...
					var unconfirmedTransaction = EIP.getUnconfirmedTransactionFromCache(1, 6, {
					...
					rows += "<tr" + (alias.tentative ? " class='tentative'" : "") + " data-alias='" + String(alias.aliasName).toLowerCase().escapeHTML() + "'><td class='alias'>" + String(alias.aliasName).escapeHTML() + "</td><td class='uri'>" + (alias.aliasURI.indexOf("http") === 0 ? "<a href='" + alias.aliasURI + "' target='_blank'>" + alias.shortAliasURI + "</a>" : alias.shortAliasURI) + "</td><td class='status'>" + alias.status + "</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#register_alias_modal' data-alias='" + String(alias.aliasName).escapeHTML() + "'>" + $.t("edit") + "</a>" + (EIP.dgsBlockPassed ? " <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#transfer_alias_modal' data-alias='" + String(alias.aliasName).escapeHTML() + "'>" + $.t("transfer") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#sell_alias_modal' data-alias='" + String(alias.aliasName).escapeHTML() + "'>" + $.t("sell") + "</a>" + (allowCancel ? " <a class='btn btn-xs btn-default cancel_alias_sale' href='#' data-toggle='modal' data-target='#cancel_alias_sale_modal' data-alias='" + String(alias.aliasName).escapeHTML() + "'>" + $.t("cancel_sale") + "</a>" : "") : "") + "</td></tr>";
				...
				EIP.dataLoadFinished($("#aliases_table"));
	...
	EIP.forms.sellAlias = function($modal) {
		...
		if (data.modal == "cancel_alias_sale") {
			...
			if (data.recipient == EIP.genesisRS) {
				if (!data.priceEFT || data.priceEFT == "0") {
					return {
						"error": $.t("error_not_specified", {
	...
	EIP.forms.sellAliasComplete = function(response, data) {
	...
	$("#sell_alias_to_specific_account, #sell_alias_to_anyone").on("click", function(e) {
		...
		if ($(this).attr("id") == "sell_alias_to_anyone") {
			$modal.find("input[name=recipient]").val(EIP.genesisRS);
	...
	$("#buy_alias_modal").on("show.bs.modal", function(e) {
		...
		EIP.sendRequest("getAlias", {
			...
			if (response.errorCode) {
				...
				if (!("priceNQT" in response)) {
				...
				} else if (typeof response.buyer != "undefined" && response.buyer != EIP.account) {
					e.preventDefault();
					...
					$modal.find("input[name=amountEFT]").val(EIP.convertToEFT(response.priceNQT)).prop("readonly", true);
				}
			}
		}, false);
	});

	EIP.forms.buyAliasError = function() {
		$("#buy_alias_modal").find("input[name=priceEFT]").prop("readonly", false);
	}

	EIP.forms.buyAliasComplete = function(response, data) {
		if (response.alreadyProcessed) {
			return;
		}

		if (EIP.currentPage != "aliases") {
			return;
		}
		...
		$("#aliases_table tbody").prepend("<tr class='tentative' data-alias='" + data.aliasName.toLowerCase() + "'><td class='alias'>" + data.aliasName + "</td><td class='uri'>" + (data.aliasURI && data.aliasURI.indexOf("http") === 0 ? "<a href='" + data.aliasURI + "' target='_blank'>" + data.aliasURI + "</a>" : data.aliasURI) + "</td><td>/</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#'>" + $.t("edit") + "</a>" + (EIP.dgsBlockPassed ? " <a class='btn btn-xs btn-default' href='#'>" + $.t("transfer") + "</a> <a class='btn btn-xs btn-default' href='#'>" + $.t("sell") + "</a>" : "") + "</td></tr>");
		...
		if (alias) {
			...
			EIP.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@eft/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	EIP.incoming.aliases = function(transactions) {
		if (EIP.hasTransactionUpdates(transactions)) {
			EIP.loadPage("aliases");
		}
	}

	EIP.forms.setAlias = function($modal) {
		var data = EIP.getFormData($modal.find("form:first"));

		data.aliasURI = $.trim(data.aliasURI).toLowerCase();

		if (data.type == "account") {
			if (!(/acct:(.*)@eft/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(EFT\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@eft";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("EFT-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@eft/i);
				if (!match) {
				...
				} else if (!/^EFT\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = EIP.accountRS;
				}
				...
				$("#register_alias_uri").val(EIP.accountRS);
			}
	...
	EIP.forms.setAliasError = function(response, data) {
		if (response && response.errorCode && response.errorCode == 8) {
			...
			EIP.sendRequest("getAlias", {
				...
				if (!response.errorCode) {
					if ("priceNQT" in response) {
						if (response.buyer == EIP.account) {
							message = $.t("alias_sale_direct_offer", {
								"eft": EIP.formatAmount(response.priceNQT)
						...
						message = "<a href='#' data-user='" + EIP.getAccountFormatted(response, "account") + "'>" + $.t("view_owner_info_q") + "</a>";
					}
	...
	EIP.forms.setAliasComplete = function(response, data) {
		if (response.alreadyProcessed) {
			return;
		}
		...
		if (EIP.currentPage == "aliases") {
			...
			if ($row.length) {
				...
				var rowToAdd = "<tr class='tentative' data-alias='" + data.aliasName.toLowerCase() + "'><td class='alias'>" + data.aliasName + "</td><td class='uri'>" + (data.aliasURI && data.aliasURI.indexOf("http") === 0 ? "<a href='" + data.aliasURI + "' target='_blank'>" + data.shortAliasURI + "</a>" : data.shortAliasURI) + "</td><td>/</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#'>" + $.t("edit") + "</a>" + (EIP.dgsBlockPassed ? " <a class='btn btn-xs btn-default' href='#'>" + $.t("transfer") + "</a> <a class='btn btn-xs btn-default' href='#'>" + $.t("sell") + "</a>" : "") + "</td></tr>";
	...
	$("#alias_search").on("submit", function(e) {
		...
		EIP.sendRequest("getAlias", {
			...
			if (response.errorCode) {
				...
				var data = {
					"account": EIP.getAccountTitle(response, "account"),
					"last_updated": EIP.formatTimestamp(response.timestamp),
				...
				$("#alias_info_table tbody").append(EIP.createInfoTable(data));
