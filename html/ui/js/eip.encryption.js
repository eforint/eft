...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.generatePublicKey = function(secretPhrase) {
		if (!secretPhrase) {
		...
		return EIP.getPublicKey(converters.stringToHexString(secretPhrase));
	}

	EIP.getPublicKey = function(secretPhrase, isAccountNumber) {
		if (isAccountNumber) {
	...
	EIP.getPrivateKey = function(secretPhrase) {
		SHA256_init();
	...
	EIP.getAccountId = function(secretPhrase) {
		return EIP.getAccountIdFromPublicKey(EIP.getPublicKey(converters.stringToHexString(secretPhrase)));

		/*	
		if (EIP.accountInfo && EIP.accountInfo.publicKey && publicKey != EIP.accountInfo.publicKey) {
			return -1;
		}
		*/
	}

	EIP.getAccountIdFromPublicKey = function(publicKey, RSFormat) {
	...
	EIP.encryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					...
					options.privateKey = converters.hexStringToByteArray(EIP.getPrivateKey(secretPhrase));
				}
				...
				if (!options.publicKey) {
					...
					try {
						options.publicKey = converters.hexStringToByteArray(EIP.getPublicKey(options.account, true));
					} catch (err) {
						var eftAddress = new EftAddress();

						if (!eftAddress.set(options.account)) {
							throw {
	...
	EIP.decryptData = function(data, options, secretPhrase) {
		try {
			return EIP.decryptNote(message, options, secretPhrase);
		} catch (err) {
			...
			if (err.errorCode && err.errorCode == 1) {
				...
				if (options.title) {
					var translatedTitle = EIP.getTranslatedFieldName(options.title).toLowerCase();
					if (!translatedTitle) {
	...
	EIP.decryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					if (!secretPhrase) {
	...
	EIP.getSharedKeyWithAccount = function(account) {
		try {
			...
			var privateKey = converters.hexStringToByteArray(EIP.getPrivateKey(secretPhrase));

			var publicKey = converters.hexStringToByteArray(EIP.getPublicKey(account, true));
	...
	EIP.signBytes = function(message, secretPhrase) {
	...
	EIP.verifyBytes = function(signature, message, publicKey) {
	...
	EIP.setEncryptionPassword = function(password) {
		_password = password;
	}

	EIP.setDecryptionPassword = function(password) {
		_decryptionPassword = password;
	}

	EIP.addDecryptedTransaction = function(identifier, content) {
		if (!_decryptedTransactions[identifier]) {
			_decryptedTransactions[identifier] = content;
		}
	}

	EIP.tryToDecryptMessage = function(message) {
		...
		try {
			if (!message.attachment.encryptedMessage.data) {
				...
				var decoded = EIP.decryptNote(message.attachment.encryptedMessage.data, {
					"nonce": message.attachment.encryptedMessage.nonce,
					"account": (message.recipient == EIP.account ? message.sender : message.recipient)
				});
			}
	...
	EIP.tryToDecrypt = function(transaction, fields, account, options) {
		...
		if (!output) {
			$.each(fields, function(key, title) {
				...
				if (encrypted) {
					...
					try {
						data = EIP.decryptNote(encrypted, {
							"nonce": nonce,
							"account": account
						});
					} catch (err) {
						...
						if (err.errorCode && err.errorCode == 1) {
							...
							if (title) {
								var translatedTitle = EIP.getTranslatedFieldName(title).toLowerCase();
								if (!translatedTitle) {
		...
		if (showDecryptionForm) {
			...
			EIP.removeDecryptionForm();
			$(outputEl).html(output).show();
		}
	}

	EIP.removeDecryptionForm = function($modal) {
	...
	EIP.decryptNoteFormSubmit = function() {
		...
		var accountId = EIP.getAccountId(password);
	...
	EIP.decryptAllMessages = function(messages, password) {
		if (!password) {
			throw {
				"message": $.t("error_passphrase_required"),
				"errorCode": 1
		...
		for (var i = 0; i < messages.length; i++) {
			...
			if (message.attachment.encryptedMessage && !_decryptedTransactions[message.transaction]) {
				try {
					var otherUser = (message.sender == EIP.account ? message.recipient : message.sender);
