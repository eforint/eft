...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.pages.messages = function(callback) {
		_messages = {};
		...
		EIP.sendRequest("getAccountTransactions+", {
			...
			if (response.transactions && response.transactions.length) {
				for (var i = 0; i < response.transactions.length; i++) {
					var otherUser = (response.transactions[i].recipient == EIP.account ? response.transactions[i].sender : response.transactions[i].recipient);
	...
	function displayMessageSidebar(callback) {
		...
		for (var i = 0; i < sortedMessages.length; i++) {
			...
			if (sortedMessage.user in EIP.contacts) {
				extra = " data-contact='" + EIP.getAccountTitle(sortedMessage, "user") + "' data-context='messages_sidebar_update_context'";
			}

			rows += "<a href='#' class='list-group-item' data-account='" + EIP.getAccountFormatted(sortedMessage, "user") + "' data-account-id='" + EIP.getAccountFormatted(sortedMessage.user) + "'" + extra + "><h4 class='list-group-item-heading'>" + EIP.getAccountTitle(sortedMessage, "user") + "</h4><p class='list-group-item-text'>" + EIP.formatTimestamp(sortedMessage.timestamp) + "</p></a>";
		}
	...
	EIP.incoming.messages = function(transactions) {
	...
	$("#messages_sidebar").on("click", "a", function(e) {
		...
		if (messages) {
			for (var i = 0; i < messages.length; i++) {
				...
				if (!messages[i].attachment) {
					...
					try {
						decoded = EIP.tryToDecryptMessage(messages[i]);
						extra = "decrypted";
					} catch (err) {
					...
					if (!messages[i].attachment["version.Message"]) {
						try {
							...
							if (messages[i].attachment.message.indexOf("feff") === 0) {
								decoded = EIP.convertFromHex16(messages[i].attachment.message);
							} else {
								decoded = EIP.convertFromHex8(messages[i].attachment.message);
							}
						}
					} else {
				...
				if (decoded !== false) {
					...
					if (extra == "to_decrypt") {
						...
						if (type == "payment") {
							decoded = "<strong>+" + EIP.formatAmount(messages[i].amountNQT) + " EFT</strong><br />" + decoded;
						}
				...
				var day = EIP.formatTimestamp(messages[i].timestamp, true);
				...
				output += "<dd class='" + (messages[i].recipient == EIP.account ? "from" : "to") + (extra ? " " + extra : "") + "'><p>" + decoded + "</p></dd>";
			}
		}

		var unconfirmedTransactions = EIP.getUnconfirmedTransactionsFromCache(1, 0, {
			"recipient": otherUser
		});
		...
		for (var i = 0; i < unconfirmedTransactions.length; i++) {
			...
			if (!unconfirmedTransaction.attachment) {
				...
				try {
					decoded = EIP.tryToDecryptMessage(unconfirmedTransaction);
					extra = "decrypted";
				} catch (err) {
					if (err.errorCode && err.errorCode == 1) {
						decoded = $.t("error_decryption_passphrase_required");
				...
				if (!unconfirmedTransaction.attachment["version.Message"]) {
					try {
						...
						if (unconfirmedTransaction.attachment.message.indexOf("feff") === 0) {
							decoded = EIP.convertFromHex16(unconfirmedTransaction.attachment.message);
						} else {
							decoded = EIP.convertFromHex8(unconfirmedTransaction.attachment.message);
						}
					}
				} else {
	...
	$("#messages_sidebar_context").on("click", "a", function(e) {
		...
		var account = EIP.getAccountFormatted(EIP.selectedContext.data("account"));
	...
	$("#messages_sidebar_update_context").on("click", "a", function(e) {
		...
		if (option == "update_contact") {
			...
			$("#send_money_recipient").val(EIP.selectedContext.data("contact")).trigger("blur");
			$("#send_money_modal").modal("show");
		}

	});

	EIP.forms.sendMessage = function($modal) {
	...
	$("#inline_message_form").submit(function(e) {
		...
		if (!EIP.rememberPassword) {
			...
			var accountId = EIP.getAccountId(data.secretPhrase);

			if (accountId != EIP.account) {
		...
		if (EIP.dgsBlockPassed && $("#inline_message_encrypt").is(":checked")) {
			data.encrypt_message = true;
		}

		if (data.message) {
			try {
				data = EIP.addMessageData(data, "sendMessage");
			} catch (err) {
		...
		EIP.sendRequest(requestType, data, function(response, input) {
			if (response.errorCode) {
				$.growl(EIP.translateServerError(response).escapeHTML(), {
					type: "danger"
				});
				...
				if (data["_extra"].message && data.encryptedMessageData) {
					EIP.addDecryptedTransaction(response.transaction, {
	...
	EIP.forms.sendMessageComplete = function(response, data) {
		data.message = data._extra.message;

		if (!(data["_extra"] && data["_extra"].convertedAccount)) {
			$.growl($.t("success_message_sent") + " <a href='#' data-account='" + EIP.getAccountFormatted(data, "recipient") + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>" + $.t("add_recipient_to_contacts_q") + "</a>", {
				"type": "success"
			});
		} else {
			$.growl($.t("success_message_sent"), {
		...
		if (EIP.currentPage == "messages") {
			...
			var $existing = $sidebar.find("a.list-group-item[data-account=" + EIP.getAccountFormatted(data, "recipient") + "]");
			...
			if ($existing.length) {
				...
				$existing.find("p.list-group-item-text").html(EIP.formatTimestamp(now));
				...
				var accountTitle = EIP.getAccountTitle(data, "recipient");
				...
				var listGroupItem = "<a href='#' class='list-group-item' data-account='" + EIP.getAccountFormatted(data, "recipient") + "'" + extra + "><h4 class='list-group-item-heading'>" + accountTitle + "</h4><p class='list-group-item-text'>" + EIP.formatTimestamp(now) + "</p></a>";
	...
	EIP.forms.decryptMessages = function($modal) {
		var data = EIP.getFormData($modal.find("form:first"));
		...
		try {
			...
			var unconfirmedMessages = EIP.getUnconfirmedTransactionsFromCache(1, 0);
			...
			success = EIP.decryptAllMessages(messagesToDecrypt, data.secretPhrase);
		} catch (err) {
		...
		if (data.rememberPassword) {
			EIP.setDecryptionPassword(data.secretPhrase);
		}
