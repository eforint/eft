...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.showAccountModal = function(account) {
		if (EIP.fetchingModalData) {
			return;
		}

		if (typeof account == "object") {
			EIP.userInfoModal.user = account.account;
		} else {
			EIP.userInfoModal.user = account;
			EIP.fetchingModalData = true;
		}

		$("#user_info_modal_account").html(EIP.getAccountFormatted(EIP.userInfoModal.user));

		$("#user_info_modal_actions button").data("account", EIP.userInfoModal.user);

		if (EIP.userInfoModal.user in EIP.contacts) {
		...
		if (EIP.fetchingModalData) {
			EIP.sendRequest("getAccount", {
				"account": EIP.userInfoModal.user
			}, function(response) {
				EIP.processAccountModalData(response);
				EIP.fetchingModalData = false;
			});
		} else {
			EIP.processAccountModalData(account);
		}

		$("#user_info_modal_transactions").show();

		EIP.userInfoModal.transactions();
	}

	EIP.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			$("#user_info_modal_account_balance").html("0");
		} else {
			$("#user_info_modal_account_balance").html(EIP.formatAmount(account.unconfirmedBalanceNQT) + " EFT");
		}
	...
	$("#user_info_modal ul.nav li").click(function(e) {
		...
		if (content.hasClass("data-loading")) {
			EIP.userInfoModal[tab]();
		}
	});

	/*some duplicate methods here...*/
	EIP.userInfoModal.transactions = function(type) {
		EIP.sendRequest("getAccountTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
					...
					if (/^EFT\-/i.test(EIP.userInfoModal.user)) {
						var receiving = (transaction.recipientRS == EIP.userInfoModal.user);
					} else {
						var receiving = (transaction.recipient == EIP.userInfoModal.user);
					}
					...
					rows += "<tr><td>" + EIP.formatTimestamp(transaction.timestamp) + "</td><td>" + transactionType + "</td><td style='width:5px;padding-right:0;'>" + (transaction.type == 0 ? (receiving ? "<i class='fa fa-plus-circle' style='color:#65C62E'></i>" : "<i class='fa fa-minus-circle' style='color:#E04434'></i>") : "") + "</td><td " + (transaction.type == 0 && receiving ? " style='color:#006400;'" : (!receiving && transaction.amount > 0 ? " style='color:red'" : "")) + ">" + EIP.formatAmount(transaction.amount) + "</td><td " + (!receiving ? " style='color:red'" : "") + ">" + EIP.formatAmount(transaction.fee) + "</td><td>" + EIP.getAccountTitle(transaction, account) + "</td></tr>";
				}

				$("#user_info_modal_transactions_table tbody").empty().append(rows);
				EIP.dataLoadFinished($("#user_info_modal_transactions_table"));
			} else {
	...
	EIP.userInfoModal.aliases = function() {
		EIP.sendRequest("getAliases", {
			...
			EIP.dataLoadFinished($("#user_info_modal_aliases_table"));
		});
	}

	EIP.userInfoModal.marketplace = function() {
		EIP.sendRequest("getDGSGoods", {
			"seller": EIP.userInfoModal.user,
			"firstIndex": 0,
			"lastIndex": 100
		}, function(response) {
			var rows = "";
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + String(good.goods).escapeHTML() + "' data-seller='" + String(EIP.userInfoModal.user).escapeHTML() + "'>" + String(good.name).escapeHTML() + "</a></td><td>" + EIP.formatAmount(good.priceNQT) + " EFT</td><td>" + EIP.format(good.quantity) + "</td></tr>";
				}
			}

			$("#user_info_modal_marketplace_table tbody").empty().append(rows);
			EIP.dataLoadFinished($("#user_info_modal_marketplace_table"));
		});
	}

	EIP.userInfoModal.assets = function() {
		EIP.sendRequest("getAccount", {
			"account": EIP.userInfoModal.user
		}, function(response) {
			if (response.assetBalances && response.assetBalances.length) {
				...
				for (var i = 0; i < response.assetBalances.length; i++) {
					if (response.assetBalances[i].balanceQNT == "0") {
						...
						if (nrAssets + ignoredAssets == response.assetBalances.length) {
							EIP.userInfoModal.addIssuedAssets(assets);
						}
						continue;
					}
				...
				EIP.userInfoModal.addIssuedAssets({});
			}
		});
	}

	EIP.userInfoModal.addIssuedAssets = function(assets) {
		EIP.sendRequest("getAssetsByIssuer", {
			...
			if (response.assets && response.assets[0] && response.assets[0].length) {
				...
				EIP.userInfoModal.assetsLoaded(assets);
			} else if (!$.isEmptyObject(assets)) {
				EIP.userInfoModal.assetsLoaded(assets);
			} else {
				$("#user_info_modal_assets_table tbody").empty();
				EIP.dataLoadFinished($("#user_info_modal_assets_table"));
			}
		});
	}

	EIP.userInfoModal.assetsLoaded = function(assets) {
		var assetArray = [];
		var rows = "";
		...
		for (var i = 0; i < assetArray.length; i++) {
			...
			rows += "<tr" + (asset.issued ? " class='asset_owner'" : "") + "><td><a href='#' data-goto-asset='" + String(asset.asset).escapeHTML() + "'" + (asset.issued ? " style='font-weight:bold'" : "") + ">" + String(asset.name).escapeHTML() + "</a></td><td class='quantity'>" + EIP.formatQuantity(asset.balanceQNT, asset.decimals) + "</td><td>" + EIP.formatQuantity(asset.quantityQNT, asset.decimals) + "</td><td>" + percentageAsset + "%</td></tr>";
		}
