...
var EIP = (function(EIP, $, undefined) {
	EIP.showRawTransactionModal = function(transaction) {
	...
	EIP.forms.broadcastTransactionComplete = function(response, data) {
		$("#parse_transaction_form").find(".error_message").hide();
		$("#transaction_operations_modal").modal("hide");
	}

	EIP.forms.parseTransactionComplete = function(response, data) {
		$("#parse_transaction_form").find(".error_message").hide();
		$("#parse_transaction_output_table tbody").empty().append(EIP.createInfoTable(response, true));
		$("#parse_transaction_output").show();
	}

	EIP.forms.parseTransactionError = function() {
		$("#parse_transaction_output_table tbody").empty();
		$("#parse_transaction_output").hide();
	}

	EIP.forms.calculateFullHashComplete = function(response, data) {
		$("#calculate_full_hash_form").find(".error_message").hide();
		$("#calculate_full_hash_output_table tbody").empty().append(EIP.createInfoTable(response, true));
		$("#calculate_full_hash_output").show();
	}

	EIP.forms.calculateFullHashError = function() {
