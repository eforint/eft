...
var EIP = (function(EIP, $, undefined) {
	...
	$(".modal").on("show.bs.modal", function(e) {
		...
		if ($visible_modal.length) {
			if ($visible_modal.hasClass("locked")) {
				...
				EIP.unlockForm($visible_modal, $btn, true);
			} else {
	...
	$(".modal").on("shown.bs.modal", function() {
		...
		EIP.showedFormWarning = false; //maybe not the best place... we assume forms are only in modals?
	});
	...
	$(".modal").on("hidden.bs.modal", function(e) {
		...
		var $feeInput = $(this).find("input[name=feeEFT]");
		...
		if ($feeInput.length) {
			...
			$(this).find(".advanced_fee").html(EIP.formatAmount(EIP.convertToNQT(defaultFee)) + " EFT");
		}

		EIP.showedFormWarning = false;
	});

	EIP.showModalError = function(errorMessage, $modal) {
	...
	$("input[name=feeEFT]").on("change", function() {
		var $modal = $(this).closest(".modal");

		var $feeInfo = $modal.find(".advanced_fee");

		if ($feeInfo.length) {
			$feeInfo.html(EIP.formatAmount(EIP.convertToNQT($(this).val())) + " EFT");
		}
	});
