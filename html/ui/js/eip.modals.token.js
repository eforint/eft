...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.forms.generateToken = function($modal) {
	...
	EIP.forms.generateTokenComplete = function(response, data) {
	...
	EIP.forms.generateTokenError = function() {
		$("#generate_token_output").hide();
	}

	EIP.forms.decodeTokenComplete = function(response, data) {
		$("#token_modal").find(".error_message").hide();

		if (response.valid) {
			$("#decode_token_output").html($.t("success_valid_token", {
				"account_link": EIP.getAccountLink(response, "account"),
				"timestamp": EIP.formatTimestamp(response.timestamp)
	...
	EIP.forms.decodeTokenError = function() {
