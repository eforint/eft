...
var EIP = (function(EIP, $, undefined) {
	$("#transactions_table, #dashboard_transactions_table").on("click", "a[data-transaction]", function(e) {
		...
		EIP.showTransactionModal(transactionId);
	});

	EIP.showTransactionModal = function(transaction) {
		...
		if (typeof transaction != "object") {
			EIP.sendRequest("getTransaction", {
				...
				EIP.processTransactionModalData(response);
			});
		} else {
			EIP.processTransactionModalData(transaction);
		}
	}

	EIP.processTransactionModalData = function(transaction) {
		var async = false;
		...
		$("#transaction_info_details_table tbody").empty().append(EIP.createInfoTable(transactionDetails, true));
		$("#transaction_info_table tbody").empty();
		...
		if (transaction.type == 0) {
			switch (transaction.subtype) {
				case 0:
					var data = {
						...
						"recipient": EIP.getAccountTitle(transaction, "recipient"),
						"sender": EIP.getAccountTitle(transaction, "sender")
					};

					$("#transaction_info_table tbody").append(EIP.createInfoTable(data));
			...
			switch (transaction.subtype) {
				case 0:
					...
					if (EIP.dgsBlockPassed) {
							...
							if (!transaction.attachment["version.Message"]) {
								try {
									...
									if (transaction.attachment.message.indexOf("feff") === 0) {
										message = EIP.convertFromHex16(transaction.attachment.message);
									} else {
										message = EIP.convertFromHex8(transaction.attachment.message);
									}
								}
							} else {
						...
						if (transaction.attachment.encryptedMessage || transaction.attachment.encryptToSelfMessage) {
							...
							if (EIP.account == transaction.recipient || EIP.account == transaction.sender) {
								var fieldsToDecrypt = {};

								if (transaction.attachment.encryptedMessage) {
									fieldsToDecrypt.encryptedMessage = $.t("encrypted_message");
								}
								if (transaction.attachment.encryptToSelfMessage && EIP.account == transaction.sender) {
									fieldsToDecrypt.encryptToSelfMessage = $.t("note_to_self");
								}

								EIP.tryToDecrypt(transaction, fieldsToDecrypt, (transaction.recipient == EIP.account ? transaction.sender : transaction.recipient), {
									"noPadding": true,
					...
					$output.append("<table><tr><td><strong>" + $.t("from") + "</strong>:&nbsp;</td><td>" + EIP.getAccountLink(transaction, "sender") + "</td></tr><tr><td><strong>" + $.t("to") + "</strong>:&nbsp;</td><td>" + EIP.getAccountLink(transaction, "recipient") + "</td></tr></table>").show();
				...
				case 1:
					...
					if (transaction.sender != EIP.account) {
						data["sender"] = EIP.getAccountTitle(transaction, "sender");
					}
				...
				case 6:
					...
					if (type != $.t("alias_sale_cancellation")) {
						data["recipient"] = EIP.getAccountTitle(transaction, "recipient");
					}
						...
						EIP.sendRequest("getAlias", {
							...
							if (!response.errorCode) {
								if (transaction.recipient != response.buyer || transaction.attachment.priceNQT != response.priceNQT) {
									...
									messageStyle = "danger";
								} else if (transaction.recipient == EIP.account) {
									message = $.t("alias_sale_direct_offer", {
										"eft": EIP.formatAmount(transaction.attachment.priceNQT)
								...
								} else if (transaction.senderRS == EIP.accountRS) {
									if (transaction.attachment.priceNQT != "0") {
			...
			switch (transaction.subtype) {
				...
				case 1:
					...
					EIP.sendRequest("getAsset", {
						...
						if (!EIP.dgsBlockPassed && transaction.attachment.comment) {
				...
				case 2:
					...
					EIP.sendRequest("getAsset", {
						...
						var data = {
							...
							"asset_name": asset.name,
							"quantity": [transaction.attachment.quantityQNT, asset.decimals],
							"price_formatted_html": EIP.formatOrderPricePerWholeQNT(transaction.attachment.priceNQT, asset.decimals) + " EFT",
							"total_formatted_html": EIP.formatAmount(EIP.calculateOrderTotalNQT(transaction.attachment.quantityQNT, transaction.attachment.priceNQT)) + " EFT"
						};
			...
			switch (transaction.subtype) {
				case 0:
					var data = {
						...
						"quantity_formatted_html": EIP.format(transaction.attachment.quantity),
						"seller": EIP.getAccountFormatted(transaction, "sender")
					};
				...
				case 1:
					...
					EIP.sendRequest("getDGSGood", {
						...
						var data = {
							...
							"seller": EIP.getAccountFormatted(goods, "seller")
						};
				...
				case 2:
					...
					EIP.sendRequest("getDGSGood", {
						...
						var data = {
							...
							"item_name": goods.name,
							"new_price_formatted_html": EIP.formatAmount(transaction.attachment.priceNQT) + " EFT",
							"seller": EIP.getAccountFormatted(goods, "seller")
						};
				...
				case 4:
					...
					EIP.sendRequest("getDGSGood", {
						...
						var data = {
							...
							"price": transaction.attachment.priceNQT,
							"quantity_formatted_html": EIP.format(transaction.attachment.quantity),
							"buyer": EIP.getAccountFormatted(transaction, "sender"),
							"seller": EIP.getAccountFormatted(goods, "seller")
						};
						...
						EIP.sendRequest("getDGSPurchase", {
							...
							if (purchase.errorCode) {
								...
								if (EIP.account == transaction.recipient || EIP.account == transaction.sender) {
									if (purchase.pending) {
										if (EIP.account == transaction.recipient) {
				...
				case 5:
					...
					EIP.sendRequest("getDGSPurchase", {
						...
						EIP.sendRequest("getDGSGood", {
							...
							data["quantity_formatted_html"] = EIP.format(purchase.quantity);

							if (purchase.quantity != "1") {
								var orderTotal = EIP.formatAmount(new BigInteger(String(purchase.quantity)).multiply(new BigInteger(String(purchase.priceNQT))));
								data["total_formatted_html"] = orderTotal + " EFT";
							}
							...
							data["buyer"] = EIP.getAccountFormatted(purchase, "buyer");
							data["seller"] = EIP.getAccountFormatted(purchase, "seller");

							if (transaction.attachment.goodsData) {
								if (EIP.account == purchase.seller || EIP.account == purchase.buyer) {
									EIP.tryToDecrypt(transaction, {
										"goodsData": {
											"title": $.t("data"),
											"nonce": "goodsNonce"
										}
									}, (purchase.buyer == EIP.account ? purchase.seller : purchase.buyer));
								} else {
							...
							} else if (EIP.account == purchase.seller && purchase.refundNQT) {
								callout = $.t("purchase_refunded");
							}
					...
					EIP.sendRequest("getDGSPurchase", {
						...
						EIP.sendRequest("getDGSGood", {
							...
							var data = {
								...
								"item_name": goods.name,
								"buyer": EIP.getAccountFormatted(purchase, "buyer"),
								"seller": EIP.getAccountFormatted(purchase, "seller")
							};

							$("#transaction_info_table tbody").append(EIP.createInfoTable(data));
							$("#transaction_info_table").show();

							if (purchase.seller == EIP.account || purchase.buyer == EIP.account) {
								EIP.sendRequest("getDGSPurchase", {
									"purchase": transaction.attachment.purchase
								}, function(purchase) {
									var callout;

									if (purchase.buyer == EIP.account) {
										if (purchase.refundNQT) {
											callout = $.t("purchase_refunded");
										}
									} else {
				...
				case 7:
					...
					EIP.sendRequest("getDGSPurchase", {
						...
						EIP.sendRequest("getDGSGood", {
							...
							data["order_total_formatted_html"] = EIP.formatAmount(orderTotal) + " EFT";
		...
		if (EIP.dgsBlockPassed && !(transaction.type == 1 && transaction.subtype == 0)) {
			if (transaction.attachment) {
				...
				if (transaction.attachment.encryptedMessage) {
					if (EIP.account == transaction.sender || EIP.account == transaction.recipient) {
						EIP.tryToDecrypt(transaction, {
							"encryptedMessage": $.t("encrypted_message"),
						}, (transaction.recipient == EIP.account ? transaction.sender : transaction.recipient), {
