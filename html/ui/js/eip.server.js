...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.setServerPassword = function(password) {
		_password = password;
	}

	EIP.sendOutsideRequest = function(url, data, callback, async) {
		if ($.isFunction(data)) {
			async = callback;
			callback = data;
			data = {};
	...
	EIP.sendRequest = function(requestType, data, callback, async) {
		...
		//convert EFT to NQT...
		try {
			var eftFields = ["feeEFT", "amountEFT", "priceEFT", "refundEFT", "discountEFT"];

			for (var i = 0; i < eftFields.length; i++) {
				var eftField = eftFields[i];
				var field = eftField.replace("EFT", "");

				if (eftField in data) {
					data[field + "NQT"] = EIP.convertToNQT(data[eftField]);
					delete data[eftField];
				}
			}
		} catch (err) {
			if (callback) {
		...
		if ("secretPhrase" in data) {
			var accountId = EIP.getAccountId(EIP.rememberPassword ? _password : data.secretPhrase);
			if (accountId != EIP.account) {
				...
				EIP.processAjaxRequest(requestType, data, callback, async);
			}
		} else {
			EIP.processAjaxRequest(requestType, data, callback, async);
		}
	...
	EIP.processAjaxRequest = function(requestType, data, callback, async) {
		if (!EIP.multiQueue) {
			EIP.multiQueue = $.ajaxMultiQueue(8);
		}
		...
		if (currentPage && EIP.currentSubPage) {
			currentSubPage = EIP.currentSubPage;
		}

		var type = ("secretPhrase" in data ? "POST" : "GET");
		var url = EIP.server + "/eft?requestType=" + requestType;
		...
		//unknown account..
		if (type == "POST" && (EIP.accountInfo.errorCode && EIP.accountInfo.errorCode == 5)) {
			if (callback) {
				callback({
					"errorCode": 2,
		...
		if (!EIP.isLocalHost && type == "POST" && requestType != "startForging" && requestType != "stopForging") {
			...
			if (EIP.accountInfo && EIP.accountInfo.publicKey) {
				data.publicKey = EIP.accountInfo.publicKey;
			} else {
				data.publicKey = EIP.generatePublicKey(secretPhrase);
				EIP.accountInfo.publicKey = data.publicKey;
			}
		} else if (type == "POST" && EIP.rememberPassword) {
		...
		if (type == "GET") {
			var ajaxCall = EIP.multiQueue.queue;
		} else {
			var ajaxCall = $.ajax;
		}
		...
		ajaxCall({
			...
			if (EIP.console) {
				EIP.addToConsole(this.url, this.type, this.data, response);
			}

			if (typeof data == "object" && "recipient" in data) {
				if (/^EFT\-/i.test(data.recipient)) {
			...
			if (secretPhrase && response.unsignedTransactionBytes && !response.errorCode && !response.error) {
				var publicKey = EIP.generatePublicKey(secretPhrase);
				var signature = EIP.signBytes(response.unsignedTransactionBytes, converters.stringToHexString(secretPhrase));

				if (!EIP.verifyBytes(signature, response.unsignedTransactionBytes, publicKey)) {
					if (callback) {
						callback({
							"errorCode": 1,
					...
					var payload = EIP.verifyAndSignTransactionBytes(response.unsignedTransactionBytes, signature, requestType, data);
					...
					if (!payload) {
						...
						if (data.broadcast == "false") {
							...
							EIP.showRawTransactionModal(response);
						} else {
							if (callback) {
								if (extra) {
									data["_extra"] = extra;
								...
								EIP.broadcastTransactionBytes(payload, callback, response, data);
							} else {
								EIP.broadcastTransactionBytes(payload, null, response, data);
							}
						}
					}
				}
				...
				if (response.errorCode || response.errorDescription || response.errorMessage || response.error) {
					response.errorDescription = EIP.translateServerError(response);
			...
			if (EIP.console) {
				EIP.addToConsole(this.url, this.type, this.data, error, true);
			}
	...
	EIP.verifyAndSignTransactionBytes = function(transactionBytes, signature, requestType, data) {
		var transaction = {};
		...
		if (!("recipient" in data)) {
			//recipient == genesis
			data.recipient = "15565913748216129843";
			data.recipientRS = "EFT-PCBM-4N9E-X8U9-FWB3J";
		}

		if (transaction.publicKey != EIP.accountInfo.publicKey) {
			return false;
		}
		...
		if (transaction.recipient !== data.recipient) {
			if (data.recipient == "15565913748216129843" && transaction.recipient == "0") {
				//ok
			} else {
				return false;
			}
	...
	EIP.broadcastTransactionBytes = function(transactionData, callback, originalResponse, originalData) {
		$.ajax({
			url: EIP.server + "/eft?requestType=broadcastTransaction",
			crossDomain: true,
			dataType: "json",
			type: "POST",
			timeout: 30000,
