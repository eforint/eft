...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.pages.settings = function() {
		for (var style in userStyles) {
			var $dropdown = $("#" + style + "_color_scheme");

			$dropdown.empty();

			$dropdown.append("<li><a href='#' data-color=''><span class='color' style='background-color:" + EIP.defaultColors[style] + ";border:1px solid black;'></span>Default</a></li>");
			...
			var color = EIP.settings[style + "_color"];
		...
		for (var key in EIP.settings) {
			if (/_warning/i.test(key) && key != "asset_transfer_warning") {
				if ($("#settings_" + key).length) {
					$("#settings_" + key).val(EIP.convertToEFT(EIP.settings[key]));
				}
			} else if (!/_color/i.test(key)) {
				if ($("#settings_" + key).length) {
					$("#settings_" + key).val(EIP.settings[key]);
				}
			}
		}

		if (EIP.settings["news"] != -1) {
	...
	EIP.updateStyle = function(type, color) {
		var css = "";
	...
	$("ul.color_scheme_editor").on("click", "li a", function(e) {
		...
		if (color) {
			EIP.updateSettings(scheme + "_color", color);
			EIP.updateStyle(scheme, color);
		} else {
			EIP.updateSettings(scheme + "_color");
			EIP.updateStyle(scheme);
		}
	});

	EIP.getSettings = function() {
		if (EIP.databaseSupport) {
			EIP.database.select("data", [{
				"id": "settings"
			}], function(error, result) {
				if (result && result.length) {
					EIP.settings = $.extend({}, EIP.defaultSettings, JSON.parse(result[0].contents));
				} else {
					EIP.database.insert("data", {
						id: "settings",
						contents: "{}"
					});
					EIP.settings = EIP.defaultSettings;
				}
				EIP.applySettings();
			});
		} else {
			if (EIP.hasLocalStorage) {
				EIP.settings = $.extend({}, EIP.defaultSettings, JSON.parse(localStorage.getItem("settings")));
			} else {
				EIP.settings = EIP.defaultSettings;
			}
			EIP.applySettings();
	...
	EIP.applySettings = function(key) {
		...
		if (!key || key == "animate_forging") {
			if (EIP.settings["animate_forging"]) {
		...
		if (!key || key == "news") {
			if (EIP.settings["news"] == 0) {
				$("#news_link").hide();
			} else if (EIP.settings["news"] == 1) {
				$("#news_link").show();
			}
		}

		if (!EIP.inApp && !EIP.downloadingBlockchain) {
			if (!key || key == "console_log") {
				if (EIP.settings["console_log"] == 0) {
					$("#show_console").hide();
				} else {
					$("#show_console").show();
				}
		...
		if (key == "24_hour_format") {
			...
			$.each($dashboard_dates, function(key, value) {
				$(this).html(EIP.formatTimestamp($(this).data("timestamp")));
			});
		}

		if (!key || key == "remember_passphrase") {
			if (EIP.settings["remember_passphrase"]) {
				EIP.setCookie("remember_passphrase", 1, 1000);
			} else {
				EIP.deleteCookie("remember_passphrase");
			}
		}
	}

	EIP.updateSettings = function(key, value) {
		if (key) {
			EIP.settings[key] = value;
		}

		if (EIP.databaseSupport) {
			EIP.database.update("data", {
				contents: JSON.stringify(EIP.settings)
			}, [{
				id: "settings"
			}]);
		} else if (EIP.hasLocalStorage) {
			localStorage.setItem("settings", JSON.stringify(EIP.settings));
		}

		EIP.applySettings(key);
	}
