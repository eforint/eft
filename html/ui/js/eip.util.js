...
var EIP = (function(EIP, $, undefined) {
	...
	EIP.formatVolume = function(volume) {
	...
	EIP.formatWeight = function(weight) {
		var digits = [],
			formattedWeight = "",
			i;
		do {
	...
	EIP.formatOrderPricePerWholeQNT = function(price, decimals) {
		price = EIP.calculateOrderPricePerWholeQNT(price, decimals, true);

		return EIP.format(price);
	}

	EIP.calculateOrderPricePerWholeQNT = function(price, decimals, returnAsObject) {
		if (typeof price != "object") {
			price = new BigInteger(String(price));
		}

		return EIP.convertToEFT(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	}

	EIP.calculatePricePerWholeQNT = function(price, decimals) {
		price = String(price);
	...
	EIP.calculateOrderTotalNQT = function(quantityQNT, priceNQT) {
	...
	EIP.calculateOrderTotal = function(quantityQNT, priceNQT) {
		if (typeof quantityQNT != "object") {
			quantityQNT = new BigInteger(String(quantityQNT));
		}
		...
		return EIP.convertToEFT(quantityQNT.multiply(priceNQT));
	}

	EIP.calculatePercentage = function(a, b) {
		a = new Big(String(a));
		b = new Big(String(b));
	...
	EIP.convertToEFT = function(amount, returnAsObject) {
		var negative = "";
		var afterComma = "";
	...
	EIP.amountToPrecision = function(amount, decimals) {
	...
	EIP.convertToNQT = function(currency) {
	...
	EIP.convertToQNTf = function(quantity, decimals, returnAsObject) {
	...
	EIP.convertToQNT = function(quantity, decimals) {
		quantity = String(quantity);
	...
	EIP.format = function(params, no_escaping) {
	...
	EIP.formatQuantity = function(quantity, decimals, no_escaping) {
		return EIP.format(EIP.convertToQNTf(quantity, decimals, true), no_escaping);
	}

	EIP.formatAmount = function(amount, round, no_escaping) {
		...
		if (typeof amount == "object") {
			var params = EIP.convertToEFT(amount, true);
	...
	EIP.formatTimestamp = function(timestamp, date_only) {
		if (typeof timestamp == "object") {
			var date = timestamp;
		} else {
			var date = new Date(Date.UTC(2014, 9, 23, 12, 0, 0, 0) + timestamp * 1000);
		}
		...
		if (!isNaN(date) && typeof(date.getFullYear) == 'function') {
			...
			if (!date_only) {
				...
				if (!EIP.settings["24_hour_format"]) {
					hours = hours % 12;
				}
				if (hours < 10) {
					hours = "0" + hours;
	...
	EIP.formatTime = function(timestamp) {
		var date = new Date(Date.UTC(2014, 9, 23, 12, 0, 0, 0) + timestamp * 1000);
	...
	EIP.convertToHex16 = function(str) {
		var hex, i;
		var result = "";
	...
	EIP.convertFromHex16 = function(hex) {
		var j;
	...
	EIP.convertFromHex8 = function(hex) {
	...
	EIP.convertToHex8 = function(str) {
		var hex = '';
	...
	EIP.getFormData = function($form, unmodified) {
	...
	EIP.getAccountLink = function(object, acc) {
		if (typeof object[acc + "RS"] == "undefined") {
			return "/";
		} else {
			return "<a href='#' data-user='" + String(object[acc + "RS"]).escapeHTML() + "' class='user-info'>" + EIP.getAccountTitle(object, acc) + "</a>";
		}
	}

	EIP.getAccountTitle = function(object, acc) {
		...
		if (formattedAcc == EIP.account || formattedAcc == EIP.accountRS) {
			return $.t("you");
		} else if (formattedAcc in EIP.contacts) {
			return EIP.contacts[formattedAcc].name.escapeHTML();
		} else {
			return String(formattedAcc).escapeHTML();
		}
	}

	EIP.getAccountFormatted = function(object, acc) {
		var type = typeof object;
	...
	EIP.setupClipboardFunctionality = function() {
		...
		if (EIP.inApp) {
			$el.on("click", function() {
				parent.postMessage({
					"type": "copy",
					"text": EIP.getClipboardText($(this).data("type"))
				}, "*");
			...
			clipboard.on("dataRequested", function(client, args) {
				client.setText(EIP.getClipboardText($(this).data("type")));
			});
	...
	EIP.getClipboardText = function(type) {
		switch (type) {
			case "account_rs":
				return EIP.accountRS;
				break;
			case "message_link":
				return document.URL.replace(/#.*$/, "") + "#message:" + EIP.account;
				break;
			case "send_link":
				return document.URL.replace(/#.*$/, "") + "#send:" + EIP.account;
				break;
			case "asset_id":
	...
	EIP.dataLoaded = function(data, noPageLoad) {
		var $el = $("#" + EIP.currentPage + "_contents");

		if ($el.length) {
			$el.empty().append(data);
		} else {
			$el = $("#" + EIP.currentPage + "_table");
			$el.find("tbody").empty().append(data);
		}

		EIP.dataLoadFinished($el);
	...
	EIP.dataLoadFinished = function($el, fadeIn) {
	...
	EIP.createInfoTable = function(data, fixed) {
		var rows = "";
		...
		*/
		...
		for (var key in data) {
			...
			if (/_formatted_html$/i.test(key)) {
				...
				if ($.isArray(value)) {
					value = EIP.formatQuantity(value[0], value[1]);
				} else {
					value = EIP.formatQuantity(value, 0);
				}
			} else if (key == "price" || key == "total" || key == "amount" || key == "fee" || key == "refund" || key == "discount") {
				value = EIP.formatAmount(new BigInteger(String(value))) + " EFT";
			} else if (key == "sender" || key == "recipient" || key == "account" || key == "seller" || key == "buyer") {
				value = "<a href='#' data-user='" + String(value).escapeHTML() + "'>" + EIP.getAccountTitle(value) + "</a>";
			} else {
	...
	EIP.formatStyledAmount = function(amount, round) {
		var amount = EIP.formatAmount(amount, round);
	...
	EIP.getUnconfirmedTransactionFromCache = function(type, subtype, fields) {
		return EIP.getUnconfirmedTransactionsFromCache(type, subtype, fields, true);
	}

	EIP.getUnconfirmedTransactionsFromCache = function(type, subtype, fields, single) {
			...
			if (fields) {
				for (var key in fields) {
					if (unconfirmedTransaction[key] == fields[key]) {
						if (single) {
							return EIP.completeUnconfirmedTransactionDetails(unconfirmedTransaction);
						} else {
		...
		if (single || unconfirmedTransactions.length == 0) {
			...
			$.each(unconfirmedTransactions, function(key, val) {
				unconfirmedTransactions[key] = EIP.completeUnconfirmedTransactionDetails(val);
			});
	...
	EIP.completeUnconfirmedTransactionDetails = function(unconfirmedTransaction) {
	...
	EIP.hasTransactionUpdates = function(transactions) {
		return ((transactions && transactions.length) || EIP.unconfirmedTransactionsChange);
	}

	EIP.showMore = function($el) {
		if (!$el) {
			$el = $("#" + EIP.currentPage + "_contents");
			if (!$el.length) {
				$el = $("#" + EIP.currentPage + "_table");
			}
		}
	...
	EIP.showFullDescription = function($el) {
		$el.addClass("open").removeClass("closed");
		$el.find(".description_toggle").text("Less...");
	}

	EIP.showPartialDescription = function($el) {
	...
	$("body").on(".description_toggle", "click", function(e) {
		...
		if ($(this).closest(".description").hasClass("open")) {
			EIP.showPartialDescription();
		} else {
			EIP.showFullDescription();
		}
	});
	...
	EIP.setCookie = function(name, value, days) {
		var expires;
	...
	EIP.deleteCookie = function(name) {
		EIP.setCookie(name, "", -1);
	}

	EIP.translateServerError = function(response) {
			...
			case 3:
				...
				if (match && match[1]) {
					return $.t("error_not_specified", {
						"name": EIP.getTranslatedFieldName(match[1]).toLowerCase()
					}).capitalize();
				}
				...
				if (match && match[1]) {
					...
					$.each(fieldNames, function(fieldName) {
						translatedFieldNames.push(EIP.getTranslatedFieldName(fieldName).toLowerCase());
					});
			...
			case 4:
				...
				if (match && match[1]) {
					return $.t("error_incorrect_name", {
						"name": EIP.getTranslatedFieldName(match[1])
					}).capitalize();
				} else {
					return response.errorDescription;
				}
	...
	EIP.getTranslatedFieldName = function(name) {
