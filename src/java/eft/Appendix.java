...
public interface Appendix {
    ...
    static abstract class AbstractAppendix implements Appendix {
        ...
        AbstractAppendix() {
            ...
            this.version = (byte)(Eft.getBlockchain().getHeight() < Constants.DIGITAL_GOODS_STORE_BLOCK ? 0 : 1);
        }
        ...
        abstract void validate(Transaction transaction) throws EftException.ValidationException;
    ...
    public static class Message extends AbstractAppendix {
        ...
        static Message parse(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws EftException.ValidationException {
            ...
            if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new EftException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
        }

        Message(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        @Override
        void validate(Transaction transaction) throws EftException.ValidationException {
            if (this.isText && transaction.getVersion() == 0) {
                throw new EftException.NotValidException("Text messages not yet enabled");
            }
            if (transaction.getVersion() == 0 && transaction.getAttachment() != Attachment.ARBITRARY_MESSAGE) {
                throw new EftException.NotValidException("Message attachments not enabled for version 0 transactions");
            }
            if (message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new EftException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    abstract static class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws EftException.ValidationException {
            super(buffer, transactionVersion);
        ...
        private AbstractEncryptedMessage(JSONObject attachmentJSON, JSONObject encryptedMessageJSON) throws EftException.ValidationException {
        ...
        void validate(Transaction transaction) throws EftException.ValidationException {
            if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new EftException.NotValidException("Max encrypted message length exceeded");
            }
            if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                ...
                throw new EftException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
            }
        }
    ...
    public static class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        static EncryptedMessage parse(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws EftException.ValidationException {
            super(buffer, transactionVersion);
        }

        EncryptedMessage(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        void validate(Transaction transaction) throws EftException.ValidationException {
            ...
            if (! transaction.getType().hasRecipient()) {
                throw new EftException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
            if (transaction.getVersion() == 0) {
                throw new EftException.NotValidException("Encrypted message attachments not enabled for version 0 transactions");
            }
        }
    ...
    public static class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        static EncryptToSelfMessage parse(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws EftException.ValidationException {
            super(buffer, transactionVersion);
        }

        EncryptToSelfMessage(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        void validate(Transaction transaction) throws EftException.ValidationException {
            super.validate(transaction);
            if (transaction.getVersion() == 0) {
                throw new EftException.NotValidException("Encrypt-to-self message attachments not enabled for version 0 transactions");
            }
        }
    ...
    public static class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        static PublicKeyAnnouncement parse(JSONObject attachmentData) throws EftException.ValidationException {
        ...
        PublicKeyAnnouncement(JSONObject attachmentData) throws EftException.ValidationException {
            super(attachmentData);
        ...
        void validate(Transaction transaction) throws EftException.ValidationException {
            if (! transaction.getType().hasRecipient()) {
                throw new EftException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (publicKey.length != 32) {
                throw new EftException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
            }
            Long recipientId = transaction.getRecipientId();
            if (! Account.getId(this.publicKey).equals(recipientId)) {
                throw new EftException.NotValidException("Announced public key does not match recipient accountId");
            }
            if (transaction.getVersion() == 0) {
                throw new EftException.NotValidException("Public key announcements not enabled for version 0 transactions");
            }
            ...
            if (recipientAccount != null && recipientAccount.getPublicKey() != null && ! Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
                throw new EftException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
