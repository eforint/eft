...
final class BlockImpl implements Block {
    ...
    BlockImpl(int version, int timestamp, Long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
        ...
        if (transactions.size() > Constants.MAX_NUMBER_OF_TRANSACTIONS) {
            throw new EftException.NotValidException("attempted to create a block with " + transactions.size() + " transactions");
        }

        if (payloadLength > Constants.MAX_PAYLOAD_LENGTH || payloadLength < 0) {
            throw new EftException.NotValidException("attempted to create a block with payloadLength " + payloadLength);
        }
        ...
        for (Transaction transaction : this.blockTransactions) {
            if (transaction.getId() < previousId) {
                throw new EftException.NotValidException("Block transactions are not sorted!");
            }
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws EftException.ValidationException {
        ...
        for (Object transactionData : transactionsData) {
            ...
            if (blockTransactions.put(transaction.getId(), transaction) != null) {
                throw new EftException.NotValidException("Block contains duplicate transactions: " + transaction.getStringId());
            }
        }
    ...
    byte[] getBytes() {
        ...
        if (version < 3) {
            buffer.putInt((int)(totalAmountNQT / Constants.ONE_EFT));
            buffer.putInt((int)(totalFeeNQT / Constants.ONE_EFT));
        } else {
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            BlockImpl previousBlock = (BlockImpl)Eft.getBlockchain().getBlock(this.previousBlockId);
            ...
            Account account = Account.getAccount(getGeneratorId());
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceEFT();
