...
final class BlockchainProcessorImpl implements BlockchainProcessor {
    ...
    private static final byte[] CHECKSUM_TRANSPARENT_FORGING = new byte[]{29, -30, 9, 46, -61, -128, 87, 123, 75, 90, -47, 126, -99, 36, -125, -108, 53, -50, 18, -19, 47, -35, 16, 68, 87, -37, -20, 7, 79, -85, 46, -13};
    private static final byte[] CHECKSUM_NQT_BLOCK = new byte[]{29, -30, 9, 46, -61, -128, 87, 123, 75, 90, -47, 126, -99, 36, -125, -108, 53, -50, 18, -19, 47, -35, 16, 68, 87, -37, -20, 7, 79, -85, 46, -13};
                    ...
                    while (forkBlocks.size() < 1440 && requestCount++ < 10) {
                        ...
                        synchronized (blockchain) {
                            ...
                            for (Object o : nextBlocks) {
                                ...
                                } catch (EftException.NotCurrentlyValidException e) {
    ...
    @Override
    public void processPeerBlock(JSONObject request) throws EftException {
        ...
        try {
            ...
            for (int i = 0; i < Genesis.GENESIS_RECIPIENTS.length; i++) {
                ...
                TransactionImpl transaction = new TransactionImpl.BuilderImpl((byte) 0, Genesis.CREATOR_PUBLIC_KEY,
                        Genesis.GENESIS_AMOUNTS[i] * Constants.ONE_EFT, 0, 0, (short) 0,
            ...
            Logger.logMessage("genesisBlockID: "+genesisBlock.getId());
...
//        }
    ...
    private BlockImpl parseBlock(JSONObject blockData) throws EftException.ValidationException {
    ...
    private volatile boolean validateAtScan = Eft.getBooleanProperty("eft.forceValidate");
                ...
                while (rs.next()) {
                    try {
                        ...
                        if (! currentBlock.getId().equals(currentBlockId)) {
                            throw new EftException.NotValidException("Database blocks in the wrong order!");
                        }
                        if (validateAtScan && ! currentBlockId.equals(Genesis.GENESIS_BLOCK_ID)) {
                            if (!currentBlock.verifyBlockSignature() || !currentBlock.verifyGenerationSignature()) {
                                throw new EftException.NotValidException("Invalid block signature");
                            }
                            if (currentBlock.getVersion() != getBlockVersion(blockchain.getHeight())) {
                                throw new EftException.NotValidException("Invalid block version");
                            }
                            byte[] blockBytes = currentBlock.getBytes();
                            JSONObject blockJSON = (JSONObject) JSONValue.parse(currentBlock.getJSONObject().toJSONString());
                            if (! Arrays.equals(blockBytes, parseBlock(blockJSON).getBytes())) {
                                throw new EftException.NotValidException("Block JSON cannot be parsed back to the same block");
                            }
                            for (TransactionImpl transaction : currentBlock.getTransactions()) {
                                if (!transaction.verify()) {
                                    throw new EftException.NotValidException("Invalid transaction signature");
                                }
                                if (transaction.getVersion() != transactionProcessor.getTransactionVersion(blockchain.getHeight())) {
                                    throw new EftException.NotValidException("Invalid transaction version");
                                }
                                if (! EconomicClustering.verifyFork(transaction)) {
                                    ...
                                    //throw new EftException.NotValidException("Invalid transaction fork");
                                }
                                transaction.validateAttachment();
                                ...
                                if (currentBlock.getHeight() > Constants.NQT_BLOCK
                                    ...
                                    throw new EftException.NotValidException("Transaction bytes cannot be parsed back to the same transaction");
                                }
                                ...
                                if (! Arrays.equals(transactionBytes, transactionProcessor.parseTransaction(transactionJSON).getBytes())) {
                                    throw new EftException.NotValidException("Transaction JSON cannot be parsed back to the same transaction");
                                }
                            }
                        }
                        for (TransactionImpl transaction : currentBlock.getTransactions()) {
                        ...
                        while (rs.next()) {
                            try {
                                currentBlock = BlockDb.loadBlock(con, rs);
                                lostTransactions.addAll(currentBlock.getTransactions());
                            } catch (EftException.ValidationException ignore) {}
                        }
