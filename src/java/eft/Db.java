...
public final class Db {
    ...
    static void init() {
        long maxCacheSize = Eft.getIntProperty("eft.dbCacheKB");
        if (maxCacheSize == 0) {
            maxCacheSize = Runtime.getRuntime().maxMemory() / (1024 * 2);
        }
        String dbUrl = Constants.isTestnet ? Eft.getStringProperty("eft.testDbUrl") : Eft.getStringProperty("eft.dbUrl");
        ...
        cp.setMaxConnections(Eft.getIntProperty("eft.maxDbConnections"));
        cp.setLoginTimeout(Eft.getIntProperty("eft.dbLoginTimeout"));
        int defaultLockTimeout = Eft.getIntProperty("eft.dbDefaultLockTimeout") * 1000;
