...
public final class Generator {
    ...
    static long getHitTime(Account account, Block block) {
        return getHitTime(account.getEffectiveBalanceEFT(), getHit(account.getPublicKey(), block), block);
    }
    ...
    private static long getHitTime(long effectiveBalanceEFT, BigInteger hit, Block block) {
        return block.getTimestamp()
                + hit.divide(BigInteger.valueOf(block.getBaseTarget())
                .multiply(BigInteger.valueOf(effectiveBalanceEFT))).longValue();
    }
    ...
    private void forge(int timestamp) {
        ...
        if (Eft.getBlockchainProcessor().isScanning()) {
            return;
        }
        ...
        long effectiveBalance = account.getEffectiveBalanceEFT();
        ...
        if (! lastBlock.equals(lastBlocks.get(accountId))) {
            ...
            deadline = Math.max(getHitTime(account.getEffectiveBalanceEFT(), hit, lastBlock) - Convert.getEpochTime(), 0);
