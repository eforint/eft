...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build() throws EftException.ValidationException {
    ...
    private TransactionImpl(BuilderImpl builder) throws EftException.NotValidException {
        ...
        if ((timestamp == 0 && Arrays.equals(senderPublicKey, Genesis.CREATOR_PUBLIC_KEY))
                ? (deadline != 0 || feeNQT != 0)
                : (deadline < 1 || feeNQT < Constants.ONE_EFT)
                || feeNQT > Constants.MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.MAX_BALANCE_NQT
                || type == null) {
            throw new EftException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeNQT + ", amount: " + amountNQT);
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new EftException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.hasRecipient()) {
            if (recipientId != null || getAmountNQT() != 0) {
                throw new EftException.NotValidException("Transactions of this type must have recipient == Genesis, amount == 0");
            }
        }

        for (Appendix.AbstractAppendix appendage : appendages) {
            if (! appendage.verifyVersion(this.version)) {
                throw new EftException.NotValidException("Invalid attachment version " + appendage.getVersion()
    ...
    public byte[] getBytes() {
        try {
            ...
            if (useNQT()) {
                ...
                buffer.putInt((int) (amountNQT / Constants.ONE_EFT));
                buffer.putInt((int) (feeNQT / Constants.ONE_EFT));
                if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(byte[] bytes) throws EftException.ValidationException {
        try {
    ...
    */
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws EftException.ValidationException {
        try {
            ...
            if (transactionType == null) {
                throw new EftException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Eft.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validateAttachment() throws EftException.ValidationException {
        if (Eft.getBlockchain().getHeight() >= Constants.PUBLIC_KEY_ANNOUNCEMENT_BLOCK && type.hasRecipient() && recipientId != null) {
            Account recipientAccount = Account.getAccount(recipientId);
            if ((recipientAccount == null || recipientAccount.getPublicKey() == null) && publicKeyAnnouncement == null) {
                throw new EftException.NotCurrentlyValidException("Recipient account does not have a public key, must attach a public key announcement");
            }
        }
        for (Appendix.AbstractAppendix appendage : appendages) {
