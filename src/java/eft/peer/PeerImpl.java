...
import eft.util.CountingInputStream;
import eft.util.CountingOutputStream;
...
final class PeerImpl implements Peer {
    ...
    public int getWeight() {
        ...
        return (int)(adjustedWeight * (account.getBalanceNQT() / Constants.ONE_EFT) / Constants.MAX_BALANCE_EFT);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof EftException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException) {
    ...
    public JSONObject send(final JSONStreamAware request) {
        ...
        try {
            ...
            URL url = new URL("http://" + address + (port <= 0 ? ":" + (Constants.isTestnet ? Peers.TESTNET_PEER_PORT : Peers.DEFAULT_PEER_PORT) : "") + "/eft");
    ...
    boolean analyzeHallmark(String address, final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_EFT * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
